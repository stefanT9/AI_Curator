...
- **S-04: A new collector is walked through a first-run flow — pick 2–4 style terms, rate a starter set built from them — so they reach `/discover` already holding likes and their first deck is genuinely ranked.** — Implemented 2026-09-10, not yet archived → `context/changes/add-onboarding-flow-for-collector/`. Dissolved Open Question 2 and superseded `S-03`. Lesson: —.
