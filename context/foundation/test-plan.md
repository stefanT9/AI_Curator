...
| e2e                       | none                     | —       | Not planned. No browser driver, no jsdom, no component-testing library installed            |
...
- **End-to-end browser coverage** — no browser driver is installed and no risk in §2 requires the full deployed shape to reproduce. Re-evaluate if a risk surfaces that only manifests across the real browser, cookie and handler boundary together.
