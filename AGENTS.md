...
- `EMAIL_DRAIN_SECRET` is server-only and deliberately not `NEXT_PUBLIC_`; read it **inside** the function. Unset does **not** mean open — the route answers 503 and `drainOutbox` returns zeroes. It has to match the `email_drain_secret` vault entry, and `email_drain_url` must point at the deployed route; nothing checks either, and the only symptom of a mismatch is `email_outbox` rows staying pending. See `supabase/seed-assets/README.md` — `supabase db reset` clears the vault, so a local reset needs both entries planted again.
